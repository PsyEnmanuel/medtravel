<template>
    <q-page :class="!$isDesktop ? 'px-1.5 py-2' : insuredId ? 'px-2 py-3' : 'px-1.5 py-2 lg:px-5 lg:py-6'">
        <div class="flex justify-between lg:flex-nowrap mb-1">
            <div class="flex lg:flex-nowrap w-full gap-2">
                ...
                <InsuranceSelect class="lg:w-[250px] w-full" @setInsurance="setInsurance"
                    @clearInsurance="clearInsurance" :model-value="state.query.where.insurance" />
                <q-checkbox v-if="$me.high_profile" class="border border-dashed px-4 h-[40px]"
                ...
                <q-checkbox v-if="$me.unixroles & 1" class="border border-dashed px-4 h-[40px]"
                    v-model="state.hasPolicies" :true-value="1" :false-value="0">Sin Pólizas</q-checkbox>
            </div>
...
import InsuredWrite from "pages/insured/components/InsuredWrite.vue";
import InsuredCard from "pages/insured/components/insuredCard.vue";
import InsuranceSelect from "src/components/select/InsuranceSelect.vue";
import { useQuasar } from "quasar";
const props = defineProps({ insuredId: Number, insuredId: Number })
const tableRef = ref()
const $me = inject("$me")
...
watch(() => state.hasPolicies, (val) => {
    console.log(val);
    if (val) {
        state.query.where['isnull:policies'] = 1
        // tableRef.value.requestServerInteraction()
        // tableRef.value.clearSelection()
    } else {
        delete state.query.where['isnull:policies']
    }
})
