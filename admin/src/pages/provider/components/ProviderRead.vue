<template>
    <div :style="style">
        <div v-if="isDrawer" class="flex justify-between items-start gap-2 py-2">
            <div class="mb-2">
                <p class="text-2xl text-info">Consultar Proveedor</p>
            </div>
        ...
        <div class="flex md:flex-nowrap gap-2 break-all">
            <div class="w-full md:max-w-[200px] min-w-[200px]">
                ...
                <UploadFileManager v-else :ref_id="state.item.id" table="t_provider"
                ...
                <div class="card rounded-md flex flex-col py-2 px-3 mb-1 mt-1">
                    ...
                    <div class="flex justify-between gap-2">
                        <div>{{ state.item.provider_type }}</div>
                    </div>
                    <div class="flex justify-between gap-2">
                ...
                <q-tab-panels v-if="!state.loading" v-model="state.tab" class="bg-transparent">
                    <q-tab-panel name="edit" class="p-0">
                        <div class="card px-3 py-2">
                            <ProviderWrite isEdit :id="state.item.id" width="100%" />
                        </div>
                    </q-tab-panel>
                    ...
                    <q-tab-panel name="file" class="p-0">
                        <UploadFileManager :ref_id="state.item.id" table="t_provider" file_type="GENERAL" />
                        <FileManager :refId="state.item.id" refKey="t_provider" />
                    </q-tab-panel>
                    <q-tab-panel name="doctor" class="p-0">
                        <DoctorProviderTable :providerId="state.item.id" width="100%" />
                    </q-tab-panel>
                    <q-tab-panel name="MRN" class="p-0">
                        <InsuredProviderTable :providerId="state.item.id" width="100%" />
                    </q-tab-panel>
                    <q-tab-panel name="contact" class="p-0">
                        <ContactProviderTable :providerId="state.item.id" width="100%" />
                    </q-tab-panel>
                    <q-tab-panel name="event" class="p-0">
                        <EventTable :providerId="state.item.id" width="100%" />
                    </q-tab-panel>
            ...
            <div class="flex flex-col w-full  md:max-w-[250px] min-w-[250px]">
                <template v-if="!state.loading">
                    <CommentTable refKey="t_provider" :refId="state.item.id" />
                </template>
            </div>
        </div>
    </div>
...
import ContactProviderTable from 'src/pages/contact/components/ContactProviderTable.vue';
import InsuredProviderTable from 'src/pages/insured/components/InsuredProviderTable.vue';
...
const state = reactive({
    ...
    tabs: [
        { label: 'Editar Proveedor', value: 'edit', icon: 'fa-duotone fa-solid fa-user-pen', visible: 1 },
        { label: 'MRN', value: 'MRN', icon: 'fa-duotone fa-solid fa-hospital-user', visible: 1 },
        { label: 'Medicos', value: 'doctor', icon: 'fa-duotone fa-solid fa-user-doctor', visible: 1 },
        { label: 'Contactos', value: 'contact', icon: 'fa-duotone fa-solid fa-address-book', visible: 1 },
...
async function removeProfile() {
    try {
        const response = await $api.delete(`file/${state.item.profile.id}`, { data: { ref_key: 't_provider', ref_id: state.item.id } })
    } catch (error) {
        console.log(error);
    }
}
