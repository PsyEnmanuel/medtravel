...
import ProviderWrite from "./ProviderWrite.vue";
...
const state = reactive({
    ...
    query: {
        ...
        where: {
            c_status: props.provider.length ? 4 : 0,
            ['in:id']: props.provider.map(i => i.id)
        }
    },
    columns: [
        {
        ...
        },
        {
            ...
            sortable: true,
            format: (value, row) => {
                const index = props.provider.findIndex(i => i.id == row.id)
                if (index !== -1) {
                    return props.provider[index].MRN
                } else {
                    return null;
                }
            }
