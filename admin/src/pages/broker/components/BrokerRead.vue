<template>
    <div :style="style">
        <div v-if="isDrawer" class="flex justify-between items-start gap-2 py-2">
            <div class="mb-2">
                <p class="text-2xl text-info">Consultar Corredor</p>
            </div>
        ...
        <div class="flex md:flex-nowrap gap-2 break-all">
            <div class="w-full md:max-w-[200px] min-w-[200px]">
                ...
                <UploadFileManager v-else :ref_id="state.item.id" table="t_broker"
                ...
                <div class="card rounded-md flex flex-col py-2 px-3 mb-1 mt-1">
                    ...
                    <div class="flex justify-between gap-2">
                        <div>{{ state.item.broker_type }}</div>
                    </div>
                    <div class="flex justify-between gap-2">
                ...
                <q-tab-panels v-if="!state.loading" v-model="state.tab" class="bg-transparent">
                    <q-tab-panel name="edit" class="p-0">
                        <div class="card px-3 py-2">
                            <BrokerWrite isEdit :id="state.item.id" width="100%" />
                        </div>
                    </q-tab-panel>
                    ...
                    <q-tab-panel name="file" class="p-0">
                        <UploadFileManager :ref_id="state.item.id" table="t_broker" file_type="GENERAL" />
                        <FileManager :refId="state.item.id" refKey="t_broker" />
                    </q-tab-panel>
                    <q-tab-panel name="event" class="p-0">
                        <EventTable :brokerId="state.item.id" width="100%" />
                    </q-tab-panel>
            ...
            <div class="flex flex-col w-full  md:max-w-[250px] min-w-[250px]">
                <template v-if="!state.loading">
                    <CommentTable refKey="t_broker" :refId="state.item.id" />
                </template>
            </div>
        </div>
    </div>
...
const state = reactive({
    ...
    timeout: null,
    tabs: [
        { label: 'Editar Corredor', value: 'edit', icon: 'fa-duotone fa-solid fa-user-pen', visible: 1 },
...
async function removeProfile() {
    try {
        const response = await $api.delete(`file/${state.item.profile.id}`, { data: { ref_key: 't_broker', ref_id: state.item.id } })
    } catch (error) {
        console.log(error);
    }
}
...
watch(() => updateStore.table.t_broker, (data) => {
