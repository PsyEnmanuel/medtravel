<template>
    <q-page :class="insuredId ? 'px-2 py-3' : 'px-1.5 py-2 lg:px-5 lg:py-6'">
        ...
        <q-table :grid="!$isDesktop" hide-pagination :rows="state.rows" :columns="state.columns" row-key="id"
                ...
                <q-td :props="props">
                    <div class="flex gap-2">
                        ...
                        <q-btn flat class="button bg-default text-primary rounded-md" label="consultar" no-caps
                            @click="$router.push(`${$path.broker}/${$path.broker_consult}/${props.row.id}`)" />
                    </div>
                </q-td>
            </template>
            ...
            <template v-slot:item="props">
                <div class="q-py-xs col-xs-12 col-sm-6 col-md-4"
                    ...
                    <BrokerCard :item="props" @selectedId="state.selectedId = $event; state.dialogWrite = true;" />
                </div>
            </template>
        ...
        <q-dialog class="q-pa-none" v-model="state.dialogWrite" :position="$isDesktop ? 'right' : 'standard'"
            ...
            <q-card>
                <BrokerWrite @close="state.dialogWrite = false" isDrawer :id="state.selectedId" isEdit
        ...
        <q-dialog v-model="state.dialogCreate" :position="$isDesktop ? 'right' : 'standard'" full-height maximized
            ...
            <q-card>
                <BrokerWrite @close="state.dialogCreate = false" isDrawer :width="$isDesktop ? '1000px' : '100%'" />
            </q-card>
        </q-dialog>
    </q-page>
</template>
...
import BrokerWrite from "./BrokerWrite.vue";
import BrokerCard from "./BrokerCard.vue";
const { t } = useI18n()
const props = defineProps({ broker: Object })
const tableRef = ref()
...
const state = reactive({
    ...
    query: {
        groupBy: ['t_broker.id'],
        where: {
            c_status: props.broker.length ? 4 : 0,
            ['in:id']: props.broker.map(i => i.id)
        }
    },
    columns: [
        {
        ...
        },
        {
            ...
            sortable: true,
            format: (value, row) => {
                const index = props.broker.findIndex(i => i.id == row.id)
                if (index !== -1) {
                    return props.broker[index].MRN
                } else {
                    return null;
                }
            }
