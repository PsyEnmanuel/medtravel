<template>
    <q-page :class="!$isDesktop ? 'px-1.5 py-2' : insuredId ? 'px-2 py-3' : 'px-1.5 py-2 lg:px-5 lg:py-6'">
        ...
        <q-table :grid="!$isDesktop" :visibleColumns="state.visibleColumns" hide-pagination :rows="state.rows"
    ...
    <q-dialog class="q-pa-none" v-model="state.dialogWrite" :position="$isDesktop ? 'right' : 'standard'" full-height
        ...
        <q-card>
            <RoleWrite @close="state.dialogWrite = false" isDrawer :id="state.selectedId" isEdit
    ...
    <q-dialog v-model="state.dialogCreate" :position="$isDesktop ? 'right' : 'standard'" full-height maximized
        ...
        <q-card>
            <RoleWrite @close="state.dialogCreate = false" isDrawer :width="$isDesktop ? '400px' : '100%'" />
        </q-card>
    </q-dialog>
</template>
...
import RoleWrite from './RoleWrite.vue'
...
const state = reactive({
    visibleColumns: $me.unixroles & 1 ? ['action', 'id', 'label', 'description'] : ['id', 'label', 'description'],
    dialogWrite: false,
    dialogList: false,
    dialogCreate: false,
    selectedId: 0,
...
watch(() => updateStore.table.t_role, (data) => {
