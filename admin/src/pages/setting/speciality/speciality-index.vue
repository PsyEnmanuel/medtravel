<template>
    <q-page :class="!$isDesktop ? 'px-1.5 py-2' : insuredId ? 'px-2 py-3' : 'px-1.5 py-2 lg:px-5 lg:py-6'">
            ...
            <template v-slot:body-cell-sub="props">
                <q-td :props="props">
                    <ul class="list-inside">
                        <li v-for="s in props.row.sub" :key="s">
                            {{ s.description }}
                        </li>
                    </ul>
                </q-td>
        ...
        <q-dialog class="q-pa-none" v-model="state.dialogWrite" :position="$isDesktop ? 'right' : 'standard'" full-height
            ...
            <q-card>
                <SpecialityWrite @close="state.dialogWrite = false" isDrawer :id="state.selectedId" isEdit
        ...
        <q-dialog v-model="state.dialogCreate" :position="$isDesktop ? 'right' : 'standard'" full-height maximized
            ...
            <q-card>
                <SpecialityWrite @close="state.dialogCreate = false" isDrawer :width="$isDesktop ? '400px' : '100%'" />
            </q-card>
        </q-dialog>
    </q-page>
</template>
...
import SpecialityWrite from "./components/SpecialityWrite.vue";
const { t } = useI18n()
const tableRef = ref()
...
watch(() => updateStore.table.t_speciality, (data) => {
