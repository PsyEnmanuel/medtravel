<template>
    ...
    <div class="grid grid-cols-2 gap-2">
        ...
        <div>
            <div>
                <div class="subtitle mb-1 text-center">Agregar Analítica Nueva</div>
                <div class="subtitle mb-1">
                    <q-input class="mb-1" dense outlined v-model="state.item.description" label="Descripción"></q-input>
                    <q-select class="mb-2" dense outlined clearable v-model="state.item.$analytic_group_id" use-input
                        hide-selected fill-input input-debounce="0" :options="options.analytic_group"
                        @filter="(val, update, abort) => filterFnCategory(val, update, abort, 'analytic_group')"
                        :placeholder="$t('analytic_group')" option-value="id" option-label="description" emit-value
                        map-options>
                        ...
                        <template v-slot:after>
                            <q-btn class="bg-pink-200" rounded dense flat icon="add"
                                @click="state.dialog_analytic_group = true">
                                <q-tooltip :delay="300" self="center middle">
                                    + {{ $t('analytic_group') }}
                                </q-tooltip></q-btn>
                            <q-dialog v-model="state.dialog_analytic_group" position="right">
                                <CategoryDialog category="analytic_group" @close="state.dialog_expense_type = false" />
                            </q-dialog>
                        </template>
                    </q-select>
            ...
            <q-table table-class="table-style-1" style="max-height: 600px; overflow-y: auto;" grid hide-pagination
                :rows="state.rows" :columns="state.columns" row-key="id" ref="tableRef" @request="onRequest" flat
                selection="multiple" v-model:pagination="state.pagination" v-model:selected="state.selected"
                        ...
                        <div bordered flat class="card pb-2 relative flex flex-col justify-between hover:bg-green-200"
                            ...
                            <div class="flex flex-nowrap">
                                ...
                                <div class="pl-3 pr-8 w-full pt-2 flex flex-col overflow-hidden line-clamp-1">
                                    ...
                                    <p class="mb-4">{{ props.row.analytic_group }}</p>
                                </div>
                            </div>
        ...
        <div>
            <div class="subtitle mb-1 text-center">Analítica seleccionados</div>
            <div class="flex flex-nowrap gap-1 mb-2" v-for="(item, index) in item.analytic" :key="item.id">
                ...
                <div class="w-full flex flex-col bg-green-200 card px-3 py-1">
                    ...
                    <q-input class="text-xs" dense outlined v-model="item.description" placeholder="Analítica"
...
const options = reactive({
    analytic_group: $cats.value.analytic_group
})
...
async function addItem(row) {
    try {
        const index = props.item.analytic.findIndex(i => +row.id === +i.id)
        if (index !== -1) {
            $q.notify({
                type: "success",
                message: "Este Analítica ya esta agregado",
            });
            return;
        }
        props.item.analytic.push({
            id: String(row.id),
...
function removeRow(item) {
    const index = props.item.analytic.findIndex((i) => +i.id === +item.id);
    if (index !== -1) {
        props.item.analytic.splice(index, 1);
    }
}

async function onSubmit() {
    try {
        const response = await $api.post(`analytic`, state.item);
...
watch(() => updateStore.table.t_analytic, (data) => {
