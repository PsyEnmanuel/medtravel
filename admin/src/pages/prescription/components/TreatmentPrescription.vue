<template>
    ...
    <div class="grid grid-cols-3 gap-2">
                        ...
                        <div bordered flat class="card pb-2 relative flex flex-col justify-between hover:bg-green-200"
                            ...
                            <div class="flex flex-nowrap">
                                ...
                                <div class="pl-3 pr-8 w-full pt-2 flex flex-col overflow-hidden line-clamp-1">
                                    ...
                                    <span class="text-uppercase whitespace-pre-wrap">{{ props.row.detail }}</span>
                                </div>
                            </div>
        ...
        <div>
            <div class="subtitle mb-1 text-center">Agregar Tratamiento</div>
        ...
        <div>
            <div class="subtitle mb-1 text-center">Tratamiento seleccionados</div>
            <div class="flex flex-nowrap gap-1 mb-2" v-for="(item, index) in item.treatment" :key="item.id">
                ...
                <div class="w-full flex flex-col bg-green-200 card px-3 py-1">
                    ...
                    <q-input class="text-xs" dense outlined v-model="item.description" placeholder="Tratamiento">
                    </q-input>
...
async function addItem(row) {
    try {
        const index = props.item.treatment.findIndex(i => +row.id === +i.id)
        if (index !== -1) {
            $q.notify({
                type: "success",
...
function removeRow(item) {
    const index = props.item.treatment.findIndex((i) => i.id === item.id);
    if (index !== -1) {
        props.item.treatment.splice(index, 1);
    }
}

async function onSubmit() {
    try {
        const response = await $api.post(`treatment`, state.item);
...
watch(() => updateStore.table.t_treatment, (data) => {
