<template>
    ...
    <div class="grid grid-cols-3 gap-2">
                        ...
                        <div bordered flat class="card pb-2 relative flex flex-col justify-between hover:bg-green-200"
                            ...
                            <div class="flex flex-nowrap">
                                ...
                                <div class="pl-3 pr-8 w-full pt-2 flex flex-col overflow-hidden line-clamp-1">
                                    ...
                                    <p class="mb-4">{{ props.row.detail }}</p>
                                </div>
                            </div>
        ...
        <div>
            <div class="subtitle mb-1 text-center">Agregar Procedimiento</div>
        ...
        <div>
            <div class="subtitle mb-1 text-center">Procedimiento seleccionados</div>
            <div class="flex flex-nowrap gap-1 mb-2" v-for="(item, index) in item.mprocedure" :key="item.id">
                ...
                <div class="w-full flex flex-col bg-green-200 card px-3 py-1">
                    ...
                    <q-input class="text-xs" dense outlined v-model="item.description" placeholder="Procedimiento">
                    </q-input>
...
async function addItem(row) {
    try {
        const index = props.item.mprocedure.findIndex(i => +row.id === +i.id)
        if (index !== -1) {
            $q.notify({
                type: "success",
                message: "Este Procedimiento ya esta agregado",
            });
            return;
        }
        props.item.mprocedure.push({
            id: String(row.id),
...
function removeRow(item) {
    const index = props.item.mprocedure.findIndex((i) => i.id === item.id);
    if (index !== -1) {
        props.item.mprocedure.splice(index, 1);
    }
}

async function onSubmit() {
    try {
        const response = await $api.post(`procedure`, state.item);
...
watch(() => updateStore.table.t_procedure, (data) => {
