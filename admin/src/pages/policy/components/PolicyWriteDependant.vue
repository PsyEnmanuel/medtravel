    ...
    <div :style="style">
        <div class="flex justify-between items-start gap-2 py-2">
            <div v-if="isEdit" class="mb-2">
                <p class="text-2xl text-info">Editar Dependiente</p>
                <b class="font-bold text-brand">{{ state.item.insured_description }}</b>
                <p class="text-xs">Los campos requeridos tienen un (*)</p>
            </div>
            <div v-else>
                <p class="text-2xl text-info">Agregar <b class="font-bold text-brand">Dependiente</b></p>
        ...
        <q-form :class="isDrawer ?? 'pb-64'" ref="writeForm" @submit="onSubmit" @reset="onReset">
            ...
            <div class="flex flex-col gap-1">
                ...
                <InsuredSelect @setInsured="setInsured($event, row)" @clearInsured="clearInsured(row)"
                    :model-value="state.item.insured_description" add requiredSelect />

                <PolicySelect :titular="true" :policy_number="props.policy_number" @setPolicy="setTitular($event)"
                    @clearPolicy="clearTitular" :model-value="state.item.titular_description" label="Elegir Titular"
                    requiredSelect />
                ...
                <CategorySelect :model-value="state.item.relationship" refKey="relationship"
                    ...
                    @clearCategory="state.item.$relationship_id = null" requiredSelect />
...
const $q = useQuasar()
const writeForm = ref();
const props = defineProps({ id: Number, isEdit: Boolean, width: String, isDrawer: Boolean, policy_number: Number })
...
async function onSubmit() {
    ...
    if (state.item.$relationship_id === 157) {
        $q.notify({
            type: 'success',
            message: 'Si es dependiente no puede ser titular.'
        })
        return;
    }
    if (props.isEdit) {
