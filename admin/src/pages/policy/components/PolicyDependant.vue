    ...
    <q-page :class="insuredId ? 'px-2 py-3' : 'px-1.5 py-2 lg:px-5 lg:py-6'">
        <div class="flex justify-between md:flex-nowrap mb-2">
            <div class="flex flex-nowrap w-full gap-2">
                ...
                <q-select class="mb-2" :label="$t('policy_branch')" dense outlined clearable
                    v-model="state.query.where.$policy_branch_id" use-input hide-selected fill-input input-debounce="0"
                    :options="options.policy_branch"
                    @filter="(val, update, abort) => filterFnCategory(val, update, abort, 'policy_branch')"
                    :placeholder="$t('policy_branch')" option-value="id" option-label="description" emit-value
                    map-options>
...
import PolicyWrite from './PolicyWrite.vue'
...
const state = reactive({
    ...
    query: {
        ...
        where: {
            'ne:insured_id': props.insuredId,
            titular_id: props.insuredId ?? null,
