<template>
    <div :style="style" class="pb-32">
        <div class="flex justify-between items-start gap-2 py-2">
            <div v-if="isEdit">
                <p class="text-2xl text-info">Editar HOSPEDAJE</p>
                <b class="font-bold text-brand">{{ state.item.description }}</b>
                <p class="text-xs">Los campos requeridos tienen un (*)</p>
            </div>
            <div v-else>
                <p class="text-2xl text-info">Agregar <b class="font-bold text-brand">HOSPEDAJE</b></p>
        ...
        <q-form :class="isDrawer ?? 'pb-64'" ref="writeForm" @submit="onSubmit" @reset="onReset">
            <div class="grid lg:grid-cols-2 gap-1">
                <div class="flex flex-col gap-1">
                    ...
                    <CategorySelect :model-value="state.item.lodging_type" refKey="lodging_type"
                        @setCategory="state.item.$lodging_type_id = $event" @clearCategory="state.item.$lodging_type_id = null"
                        add multiple />
                    ...
                    <StateSelect :refId="state.item.country_id" @setState="setState" @clearState="clearState"
                        :model-value="state.item.state" :readonly="!!state.item.city_id" />
...
const options = reactive({
    lodging_type: $cats.value.lodging_type,
    binary: [
        {
            label: 'Si',
            value: 1
...
async function onSubmit() {
    if (props.isEdit) {
        const response = await $api.put(`lodging/${props.id}`, {
            ...state.item,
        });
        if (response) {
            $q.notify({
                type: 'success',
                message: 'Hospedaje Editada'
            })
        }
    } else {
