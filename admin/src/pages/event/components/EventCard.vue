<template>
    <div class="card relative">
        <div class="rounded-none px-2 py-1 relative flex flex-col gap-1 flex-nowrap line-clamp-2 text-xxs text-left h-full overflow-hidden leading-3 hover:shadow-primary cursor-pointer"
            ...
            <span class="border-t pt-1">{{ item.event_type }}</span>
            <div class="border-t pt-1 flex flex-nowrap items-center justify-between w-full text-primary font-bold">
            ...
            <span v-if="item.insured" class="border-t pt-1">{{ item.insured }}</span>
            <!-- <span v-if="item.insurance_description" class="border-t pt-1">{{ item.insurance_description }}</span> -->
            <!-- <span v-if="item.country" class="border-t pt-1">{{ item.country }}</span>
            <span v-if="item.state" class="border-t pt-1">{{ item.state }}</span>
            <span v-if="item.city" class="border-t pt-1">{{ item.city }}</span> -->
            <span class="border-t pt-1 min-h-[17px]">{{ item.provider_description }}</span>
            <div v-if="item.doctor_description" class="flex flex-col text-xxs border-t pt-1">
                <div v-for="(doctor_description, index) in item.doctor_description" :key="doctor_description">
            ...
            <div class="grid grid-cols-2 gap-1">
                ...
                <span v-if="item.request_date_format" class="border-t pt-1 text-xxs text-right">{{
                    item.request_date_format }}</span>
            </div>
        </div>
    </div>
