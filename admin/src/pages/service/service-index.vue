<template>
    ...
    <q-dialog class="q-pa-none" v-model="state.dialogWrite" :position="$isDesktop ? 'right' : 'standard'" full-height
        ...
        <q-card>
            <ServiceWrite @close="state.dialogWrite = false" isDrawer :id="state.selectedId" isEdit
    ...
    <q-dialog v-model="state.dialogCreate" :position="$isDesktop ? 'right' : 'standard'" full-height maximized
        ...
        <q-card>
            <ServiceWrite @close="state.dialogCreate = false" isDrawer :width="$isDesktop ? '400px' : '100%'" />
        </q-card>
    </q-dialog>
</template>
...
import ServiceWrite from './components/ServiceWrite.vue'
...
watch(() => updateStore.table.t_catalogue, (data) => {
