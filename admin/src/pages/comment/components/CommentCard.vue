<template>
    <div class="rounded-none px-2 py-1 relative border-b mb-2 flex flex-col gap-1 flex-nowrap line-clamp-2 text-xs text-left h-full overflow-hidden leading-3 " :class="item.id ? 'hover:shadow-primary cursor-pointer' : ''"
        @click.stop.prevent="item.id ? $emit('open', item.id) : ''">
