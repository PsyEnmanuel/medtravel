    ...
    <div v-if="!state.loading" :style="style" :class="isDrawer && 'pb-32'">
        ...
        <q-form :class="isDrawer ?? 'pb-64'" ref="writeForm" @submit="onSubmit" @reset="onReset">
            <div class="grid grid-cols-1 gap-2">
                <div class="bg-default p-1 rounded-md text-center font-bold mb-2">Comentario Administrativo</div>
                <q-checkbox v-if="isEdit && $me.unixroles & 7" class="mb-2 border-1 border-dashed" dense outlined :true-value="1" :false-value="0"
                    v-model="state.item.blocked" :label="state.item.blocked ? $t('Bloqueado') : $t('Bloquear')" :rules="[requiredInput]"
                ...
                <q-input v-if="props.isEdit" readonly dense outlined v-model="state.item.comment_state"
                    :label="$t('comment_state') + '*'" :rules="[requiredInput]" hide-bottom-space></q-input>
                <q-input v-if="props.isEdit" readonly dense outlined v-model="state.item.created_format"
                    :label="$t('created') + '*'" :rules="[requiredInput]" hide-bottom-space></q-input>
                <q-input v-if="props.isEdit" readonly dense outlined v-model="state.item.created_by"
                    :label="$t('created_by') + '*'" :rules="[requiredInput]" hide-bottom-space></q-input>
            </div>
            <template v-if="!state.item.blocked || $me.unixroles & 7">
                <div v-if="isDrawer" class="fixed bottom-0 lg:right-[10px] right-0 py-2 bg-white mt-10" :style="style">
                    <div v-if="props.isEdit" class="flex flex-nowrap gap-2">
                        <q-btn v-if="$me.unixroles & 3" class="button-icon text-lg rounded-md bg-primary text-white"
                            flat icon="fa-solid fa-close" size="xs" @click="removeRow" />
            ...
            <template v-else>
                <div v-if="isDrawer" class="disabled fixed bottom-0 lg:right-[10px] right-0 py-2 bg-white mt-10"
                    :style="style">
                    <div class="text-center mb-1 border-1 border-dashed">Para editarlo debe comunicarse con un
                        administrador</div>
                    <q-btn disabled class="button-press w-full text-lg rounded-md bg-primary text-white" flat
                        label="Bloqueado" />
                </div>
            </template>
        </q-form>
