<template>
    <q-page :class="!$isDesktop ? 'px-1.5 py-2' : CustomerId ? 'px-2 py-3' : 'px-1.5 py-2 lg:px-5 lg:py-6'">
                ...
                <q-td :props="props">
                    <div class="flex gap-2">
                        ...
                        <q-btn flat class="button bg-default text-primary rounded-md" label="consultar" no-caps
                            @click="$router.push(`${$path.customer}/${$path.customer_consult}/${props.row.id}`)" />
                    </div>
                </q-td>
            </template>
                ...
                <div class="q-py-xs col-xs-12 col-sm-6 col-md-4"
                    ...
                    <div bordered flat class="card pb-2 relative flex flex-nowrap">
                        ...
                        <div class="pl-3 pr-8 w-full pt-2 flex flex-col overflow-hidden">
                            ...
                            <span class="text-uppercase line-clamp-1 text-xxs">{{ props.row.phone }}</span>
        ...
        <q-dialog class="q-pa-none" v-model="state.dialogWrite" :position="$isDesktop ? 'right' : 'standard'"
            ...
            <q-card>
                <CustomerWrite @close="state.dialogWrite = false" isDrawer :id="state.selectedId" isEdit
        ...
        <q-dialog v-model="state.dialogCreate" :position="$isDesktop ? 'right' : 'standard'" full-height maximized
            ...
            <q-card>
                <CustomerWrite @close="state.dialogCreate = false" isDrawer :width="$isDesktop ? '1000px' : '100%'" />
            </q-card>
        </q-dialog>
    </q-page>
</template>
