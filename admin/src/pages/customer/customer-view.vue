    ...
    <q-page class="px-1.5 py-2 lg:px-5 lg:py-6">
        <div class="flex md:flex-nowrap gap-2 break-all">
            <div class="w-full md:max-w-[200px] min-w-[200px]">
                ...
                <UploadFileManager v-else :ref_id="state.item.id" table="t_customer"
                ...
                <div class="card rounded-md flex flex-col py-2 px-3 mb-1 mt-1">
                    <div class="font-semibold text-xs">#{{ state.item.code }}</div>
                ...
                <q-tab-panels v-if="!state.loading" v-model="state.tab" class="bg-transparent">
                    <q-tab-panel name="edit" class="p-0">
                        <div class="card px-3 py-2">
                            <CustomerWrite isEdit :id="state.item.id" width="100%" />
                        </div>
                    </q-tab-panel>
                    <q-tab-panel name="file" class="p-0">
                        <UploadFileManager :ref_id="state.item.id" table="t_customer" file_type="GENERAL" />
                        <FileManager :refId="state.item.id" refKey="t_customer" />
                    </q-tab-panel>
                </q-tab-panels>
            </div>
...
import CustomerWrite from './components/CustomerWrite.vue'
...
const state = reactive({
    ...
    tabs: [
        { label: 'Editar Cliente', value: 'edit', icon: 'fa-duotone fa-solid fa-user-pen', visible: 1 },
...
async function removeProfile() {
    try {
        const response = await $api.delete(`file/${state.item.profile.id}`, { data: { ref_key: 't_customer', ref_id: state.item.id } })
    } catch (error) {
        console.log(error);
    }
}
...
watch(() => updateStore.table.t_customer, (data) => {
