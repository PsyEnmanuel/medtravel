import { endOfMonth, endOfWeek, endOfYear, format, startOfMonth, startOfWeek, startOfYear, startOfYesterday, subMonths, subWeeks } from "date-fns";
...
export function disableDates(date) {
  try {
...
export function disableDatesBirthday(date) {
  try {
      return date <= format(new Date(), 'yyyy/MM/dd')
  } catch (error) {
      console.log(error);
...
export function setTimePeriodQuery(period, query, key) {
  switch (period) {
    case "today":
      query.where[key].from = format(new Date(), "dd-MM-yyyy");
      query.where[key].to = format(new Date(), "dd-MM-yyyy");
      break;
    case "yesterday":
      query.where[key].from = format(
        startOfYesterday(new Date()),
        "dd-MM-yyyy"
      );
      query.where[key].to = format(startOfYesterday(new Date()), "dd-MM-yyyy");
      break;
    case "week":
      query.where[key].from = format(startOfWeek(new Date()), "dd-MM-yyyy");
      query.where[key].to = format(endOfWeek(new Date()), "dd-MM-yyyy");
      break;
    case "lastweek":
      const lastweek = subWeeks(new Date(), 1);
      query.where[key].from = format(startOfWeek(lastweek), "dd-MM-yyyy");
      query.where[key].to = format(endOfWeek(lastweek), "dd-MM-yyyy");
      break;
    case "month":
      query.where[key].from = format(startOfMonth(new Date()), "dd-MM-yyyy");
      query.where[key].to = format(endOfMonth(new Date()), "dd-MM-yyyy");
      break;
    case "lastmonth":
      const lastMonth = subMonths(new Date(), 1);
      query.where[key].from = format(startOfMonth(lastMonth), "dd-MM-yyyy");
      query.where[key].to = format(endOfMonth(lastMonth), "dd-MM-yyyy");
      break;
    case "year":
      query.where[key].from = format(startOfYear(new Date()), "dd-MM-yyyy");
      query.where[key].to = format(endOfYear(new Date()), "dd-MM-yyyy");
      break;
    case "all":
      query.where[key].from = null;
      query.where[key].to = null;
      break;
  }
}
...
export function buildURL(params) {
  const url = new URL("https://saonas.com");
  ...
  const flattenObject = (obj, parentKey = "") => {
    Object.keys(obj).forEach((key) => {
      ...
      if (typeof obj[key] === "object" && !Array.isArray(obj[key])) {
        // If the value is an object (but not an array), recursively build the nested parameters
        flattenObject(obj[key], currentKey);
      } else if (Array.isArray(obj[key])) {
        // If the value is an array, handle it differently (in this case, join the array values)
        const arrayKey = `${currentKey}[]`;
        url.searchParams.set(arrayKey, obj[key].join(","));
      } else {
        // If the value is not an object or array, set it directly
        url.searchParams.set(currentKey, obj[key]);
      }
    });
  };
...
export function calculatePercentValue(value, percent) {
...
export function deCloneObj(obj) {
  return JSON.parse(JSON.stringify(obj));
}
...
export function objectMerge(target, source) {
  ...
  Object.keys(source).forEach((property) => {
    ...
    if (
      ...
      Object.keys(sourceProperty).length !== 0
    ) {
...
export function getPercent(numA, numB) {
  return ((numB / 100) * numA).toFixed(2);
}
