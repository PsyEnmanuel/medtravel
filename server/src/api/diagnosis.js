...
router.get("/user", async function (req, res, next) {
  try {
    ...
    const items = await pool.query("SELECT diagnosis, diagnosis_ids  FROM t_patient WHERE 1 AND`c_status` = 4 AND (`diagnosis` IS NOT NULL OR `diagnosis` != '[]') ORDER BY created ASC")
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (item.diagnosis.length) {
        for (let i = 0; i < item.diagnosis.length; i++) {
          ...
          const index = diagnosisList.findIndex(i => i.id === row.id)
          if (index === -1) {
            diagnosisList.push({
              id: row.id,
              code: row.code,
