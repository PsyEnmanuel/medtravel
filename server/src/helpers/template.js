...
export function generateConciliation({ item, itineraries, provider, files, account, user }) {
  try {

    function buildClaimDetailPdf(rows, patientName = "") {
      const header = [
    ...
    // ---- Usage (browser) ----
    // const dd = buildClaimDetailPdf(rows, "LAURA DALISSA PIMENTEL DOTTIN");
    // pdfMake.createPdf(dd).open(); // or .download("detalle-reclamacion.pdf")
